...
def delete_row_from_google_sheets(sheet_id, sheet_name, employee_name, from_date, to_date):
    ...
    the provided from_date and to_date, if the leave is UPCOMING or starts today, but not if REDEEMED.
    ...
    try:
        ...
        # Identify rows to delete (exact match only)
        rows_to_delete = []
        for i, row in enumerate(all_rows):
            ...
            # Check for an exact match of dates
            if (row_from_date == from_date and row_to_date == to_date):
                ...
                if row[7].strip().upper() == "REDEEMED":
                    ...
                    return False, f"Cannot cancel leave from {from_date} to {to_date}: the leave has already been redeemed."
                if row_from_dt < today_date_dt and row[7].strip().upper() != "UPCOMING":
                    ...
                    return False, f"Cannot cancel leave from {from_date} to {to_date}: the leave is in the past and not marked as UPCOMING."

                rows_to_delete.append(i + 1)  # 1-based index for gspread
            else:
                logging.info(f"📝 No exact match for {employee_name} from {from_date} to {to_date}. Row {i+1} has {row_from_date} to {row_to_date}.")

        if not rows_to_delete:
            logging.info(f"📝 No exactly matching cancellable leaves found for {employee_name} from {from_date} to {to_date} in sheet_id={sheet_id}, sheet_name={sheet_name}.")
            return False, f"No leave entries found for {employee_name} from {from_date} to {to_date} to cancel. Please ensure the dates match exactly with a previously logged leave."
        ...
        return True, f"Leave cancellation for {employee_name} from {from_date} to {to_date} processed."
        ...
        return False, f"Error cancelling leave for {employee_name} from {from_date} to {to_date}: {str(e)}"